...
const (
	...
	TableNamePoint = "aid_activity_point"
	Module         = "activity"
)

func init() {
	interfaces.S.Activity = NewService()
}

func NewService() service.ActivityService {
...
func (s *activityService) ListByAID(aid int64, limit int) ([]*entities.Activity, error) {
	...
	if err != nil {
		log.DefaultLogger().Errorf("list error: %v", err)
		return nil, err
	}
...
func (s *activityService) ListByAIDs(aids []int64) (map[int64][]*entities.Activity, error) {
	...
	if err != nil {
		log.DefaultLogger().Errorf("list by id error: %v", err)
		return nil, err
	}
...
func (s *activityService) GetOneByID(id int64) (*entities.Activity, error) {
	...
	if err != nil {
		log.DefaultLogger().Errorf("get activity by id error: %v", err)
		return nil, err
	}
...
func (s *activityService) GetManyByIDs(ids []int64) ([]*entities.Activity, error) {
	...
	if err != nil {
		log.DefaultLogger().Errorf("list activity by ids error: %v", err)
		return nil, err
	}
...
func (s *activityService) CreateWithSession(session *xorm.Session, a *entities.Activity) error {
	...
	if err != nil {
		log.DefaultLogger().Errorf("insert error: %v", err)
		return err
	}
...
func (s *activityService) SaveActivityDeviceGot(event *events.DeviceGotEvent) ([]*entities.DealPointsEventRst, error) {
	...
	return rst, db.Begin(func(session *xorm.Session) error {
		...
		if err != nil {
			log.DefaultLogger().Errorf("onDeviceGot error: %v", err)
			return err
		}
