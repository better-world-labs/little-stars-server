...
func init() {
	//TODO 后边把 UserService 也注册进去
	interfaces.S.Task = &Service{user: user.NewService(nil)}
}
...
func (s Service) GetUserTaskStat(userId int64) (*service.UserTaskStat, error) {
	return findUserTaskStat(userId)
}

// GenJobsByUserLocation 根据用户的定位信息生成Job
func (s Service) GenJobsByUserLocation(userId int64, coordinate location.Coordinate) ([]*service.Job, error) {
	...
	// 腾讯只保存ID
	//list, err := tencent.SearchMapDevice(coordinate.Longitude, coordinate.Latitude, Nearby, 0, AedCount)
...
func (s Service) CompleteJob(userId int64, jobId int64) error {
	return completeJob(userId, jobId)
}
...
func (Service) IsTodayHasBubble(userId int64) (bool, error) {
	...
	exist, err := db.SQL(`
		select
			count(if(status = 0, 1, null)) total,
			count(if(status = 10 and completed_at > CURRENT_DATE(), 1, null)) as today
		from task_job
		where
			user_id = ?
			and (
				is_time_limit = 0
				or (begin_limit <= now() and now() <= end_limit)
			)
	`, userId).Get(&count)
	if err != nil {
		return false, err
