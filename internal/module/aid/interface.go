...
type Service interface {
	...
	// @return int64 ID
	// @return error 错误
	PublishHelpInfo(accountID int64, dto *PublishDTO) (int64, []*entities.DealPointsEventRst, error)
	...
	// @return 数据 Map
	// @return error 错误
	GetHelpImagesByHelpInfoIDs(helpInfoIDs []int64) (map[int64][]*HelpImage, error)

	// GetHelpImagesByHelpInfoIDsAsync 读取多个求助信息的图片异步版
	// @param helpInfoIDs 求助信息 ID
	// @return 数据 Map
	// @return func 结果闭包
	GetHelpImagesByHelpInfoIDsAsync(helpInfoIDs []int64) func() (map[int64][]*HelpImage, error)
	...
	// @param position 位置
	// @return 错误
	GetHelpInfoComposedByID(id int64, position *location.Coordinate) (*HelpInfoComposedDTO, bool, error)
}
